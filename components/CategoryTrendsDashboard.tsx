    ...
    return (
        <div className="p-8 bg-gray-50 dark:bg-hunted-dark min-h-screen font-sans">
            <div className="max-w-7xl mx-auto">
                <div className="mb-12">
                    <h1 className="text-3xl font-bold mb-2 text-gray-900 dark:text-hunted-text">Market Understanding</h1>
                    ...
                    {data.map((category, index) => {
                        ...
                        return (
                            <Link
                                ...
                                key={category.topic}
                                className={`bg-white dark:bg-hunted-card rounded-xl p-6 border border-gray-100 dark:border-hunted-border shadow-sm hover:shadow-lg hover:scale-[1.02] transition-all duration-200 cursor-pointer group ${isFeatured ? 'md:col-span-2 lg:col-span-2 row-span-2' : ''}`}
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <h3 className={`font-bold text-gray-900 dark:text-hunted-text truncate pr-2 ${isFeatured ? 'text-2xl' : 'text-lg'}`} title={category.topic}>{category.topic}</h3>
                                ...
                                <div className="flex items-center justify-between text-xs text-gray-400 dark:text-hunted-muted mb-4">
                                    <span>{category.timeSeriesData[0]?.month}</span>
                                    <span>{category.timeSeriesData[category.timeSeriesData.length - 1]?.month}</span>
                                </div>

                                <div className="pt-4 border-t border-gray-100 dark:border-hunted-border flex justify-between items-center">
                                    <p className="text-sm text-gray-500 dark:text-hunted-muted line-clamp-2">

...
export default function DeskLayout({ children, hideSidebar = false }: { children: React.ReactNode, hideSidebar?: boolean }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-app)] flex font-sans text-gray-300 selection:bg-[#FF6154] selection:text-white">
      ...
      <div className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${hideSidebar ? '' : (isCollapsed ? 'lg:ml-20' : 'lg:ml-64')}`}>
        {/* Top Header */}
        <header className={`fixed top-0 right-0 left-0 h-16 bg-[var(--bg-app)]/80 backdrop-blur-md border-b border-[var(--border-subtle)] flex items-center justify-between px-4 lg:px-8 z-40 transition-all duration-300 ${hideSidebar ? '' : (isCollapsed ? 'lg:left-20' : 'lg:left-64')}`}>
          {!hideSidebar && (
            <button
